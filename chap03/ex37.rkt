;; The first three lines of this file were inserted by DrRacket. They record metadata
;; about the language level of this file in a form that our tools can easily process.
#reader(lib "htdp-beginner-reader.ss" "lang")((modname ex34) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))

; Problem statement
; Data definitions
; Signature, Purposes, Header(random return value from its class)
; Examples
; Templates (for function body, what parameter to compute)
; Code
; Test

; string-rest, remove the first character from a string
; string to represent string
; string -> character
; remove the first char from a string
; create a substring that abandon the first char
; given: "hao hao dau xanh", expect: "ao hao dau xanh"
; given: "hi", epect: "i"
; given: "", epect: ""
; given: "h", expect: ""

(define (string-rest str) (if (<= (string-length str) 0) "" (substring str 1 (string-length str))))
